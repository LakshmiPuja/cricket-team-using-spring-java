...
public class PlayerService implements PlayerRepository {
    ...
    public Player updatePlayer(int playerId,Player player){
        Player exixstingPlayer = team.get(playerId);
        if (exixstingPlayer == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        if (player.getPlayerName() != null) exixstingPlayer.setPlayerName(player.getPlayerName());
        if (player.getJerseyNumber() != 0) exixstingPlayer.setJerseyNumber(player.getJerseyNumber());
        if (player.getRole()!= null) exixstingPlayer.setRole(player.getRole());
        return exixstingPlayer;
    }
